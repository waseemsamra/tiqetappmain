...
import { Calendar, Ticket, User, AtSign, MapPin, Send, Home, CheckCircle, Phone, Loader2 } from 'lucide-react';
import Link from 'next/link';
import { Button } from '@/components/ui/button';
import { useEffect, useState } from 'react';
import type { Booking } from '@/types';
import { findBookingByReferenceAndEmail } from '@/app/actions';
...
function BookingVoucherPage() {
    ...
    useEffect(() => {
        setVoucherUrl(window.location.href.split('?')[0] + `?ref=${bookingRef}&email=${email}`);
        ...
        const fetchBooking = async () => {
            setIsLoading(true);
            const data = await findBookingByReferenceAndEmail(bookingRef, email);
            if (data) {
                setBooking(data);
                if (isNew) {
                    event({
    ...
    const customerIdentifier = booking.guest_name || booking.guest_email || `User ID: ${booking.user_id}`;
    const customerLabel = booking.guest_name ? "Guest Name" : (booking.guest_email ? "Guest Email" : "Booked by");
